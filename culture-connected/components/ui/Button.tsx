...
export function Button({ href, children, variant = 'solid', className = '' }: ButtonProps) {
  return (
    <Link
      href={href}
      className={`inline-flex items-center justify-center whitespace-nowrap rounded-full font-sora font-semibold no-underline ${variantClasses[variant]} ${className}`}
    >
      {children}
    </Link>
  );
